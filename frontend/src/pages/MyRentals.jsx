  ...
  if (rentals.length === 0) {
    return (
      <div className="text-center py-12 md:py-16 px-4">
        <FaServer className="text-5xl md:text-6xl text-gray-600 mx-auto mb-4" />
        <h2 className="text-xl md:text-2xl font-bold mb-2">No Active Rentals</h2>
        <p className="text-gray-400 mb-6 text-sm md:text-base">Start mining by renting your first miner</p>
        <a
          href="/miners"
          className="inline-block bg-gradient-primary hover:opacity-90 px-8 py-4 md:py-3 rounded-lg font-semibold transition min-h-[48px]"
        >
          Browse Miners
        </a>
      </div>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-400 text-sm md:text-base">Track your mining contracts and earnings</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {rentals.map((rental) => (
          <div key={rental.id} className="glass-card p-4 md:p-6 rounded-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-3 md:gap-4">
                <div className="bg-gradient-primary p-3 rounded-lg">
                  <FaServer className="text-xl md:text-2xl text-white" />
                </div>
                <div>
                  <h3 className="text-lg md:text-xl font-bold">{rental.miner_name || `Miner #${rental.miner_id}`}</h3>
                  <p className="text-sm text-gray-400">{rental.hashrate_allocated} TH/s allocated</p>
                </div>
              </div>
              <div className="flex items-center gap-2 self-start sm:self-auto">
                {rental.is_active ? (
                  <span className="flex items-center gap-2 bg-green-900 text-green-300 px-3 md:px-4 py-2 rounded-full text-xs md:text-sm">
                    <FaCheckCircle /> Active
                  </span>
                ) : (
                  <span className="flex items-center gap-2 bg-yellow-900 text-yellow-300 px-3 md:px-4 py-2 rounded-full text-xs md:text-sm">
                    <FaClock /> Pending
                  </span>
                )}
              </div>
